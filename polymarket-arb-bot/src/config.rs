...
pub struct StrategyConfig {
    ...
    /// Min price diff (higher - lower team) to use team+draw logic. If diff < this, monitor for rising-team buy.
    #[serde(default = "default_min_team_price_diff")]
    pub min_team_price_diff: f64,
    /// When monitoring weak-diff match: buy team if its price rises >= this % from previous (e.g. 0.15 = 15%).
    #[serde(default = "default_rising_buy_threshold_pct")]
    pub rising_buy_threshold_pct: f64,
    /// When holding single team: sell if price rises >= this % from buy price (e.g. 0.15 = 15%).
    #[serde(default = "default_single_team_profit_rise_pct")]
    pub single_team_profit_rise_pct: f64,
    /// When holding single team: sell if no price change for this many consecutive polls (e.g. 20).
    #[serde(default = "default_stale_price_data_points")]
    pub stale_price_data_points: u32,
    #[serde(default)]
...
fn default_min_team_price_diff() -> f64 {
    0.2 // Use team+draw only when higher-lower >= 0.2; else monitor for rising
}
fn default_rising_buy_threshold_pct() -> f64 {
    0.15 // Buy team when its price rises 15% from previous (weak-diff monitoring)
}
fn default_single_team_profit_rise_pct() -> f64 {
    0.15 // Sell single team when price rises 15% from buy
}
fn default_stale_price_data_points() -> u32 {
    20 // Sell single team if no price change for 20 polls
}
    ...
    fn default() -> Self {
        Self {
            ...
            strategy: StrategyConfig {
                ...
                min_team_price_diff: default_min_team_price_diff(),
                rising_buy_threshold_pct: default_rising_buy_threshold_pct(),
                single_team_profit_rise_pct: default_single_team_profit_rise_pct(),
                stale_price_data_points: default_stale_price_data_points(),
