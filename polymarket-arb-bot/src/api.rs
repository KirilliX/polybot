...
impl PolymarketApi {
    ...
    /// Fetch best bid price for a token from CLOB order book.
    /// When we SELL, we sell at the bid (what buyers offer). Only sell if bid >= buy_price.
    /// Bids are typically DESCENDING (highest first). Best bid = max.
    pub async fn get_best_bid(&self, token_id: &str) -> Result<Option<f64>> {
        let url = format!("{}/book", self.clob_url);
        let response = self.client
            .get(&url)
            .query(&[("token_id", token_id)])
        ...
        let json: Value = response.json().await.unwrap_or(Value::Null);
        let bids = json.get("bids").and_then(|b| b.as_array());
        let best_bid = bids.and_then(|arr| {
            arr.iter()
                .filter_map(|o| o.get("price").and_then(|p| p.as_str()).and_then(|s| s.parse::<f64>().ok()))
                .max_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))
        });
        // Placeholder when empty: bid ~0.01. Use last_trade_price as fallback.
        if best_bid.map(|b| b < 0.05).unwrap_or(true) {
            let last = json.get("last_trade_price")
                .and_then(|p| p.as_str())
                .and_then(|s| s.parse::<f64>().ok());
            if let Some(ltp) = last {
